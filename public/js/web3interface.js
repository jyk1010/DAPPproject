...
// const contract_address = "0xcfd4F0d5D6Cc1B0F6627a1f510b995dE042929f5"; // 따옴표 안에 주소값 복사 붙여넣기

const contract_address = "0x7E6C38545d05A7Bfb59Df74E3B08ad764729ba88"; // 따옴표 안에 주소값 복사 붙여넣기
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById('InActive').addEventListener("click", () => {
    markRoomAsInactive(document.getElementById('roomId').value);
  });
  document.getElementById('ClearAll').addEventListener("click", () => {
    intializeRoomShare(document.getElementById('roomId').value);
  });
...
const _shareRoom = async (name, location, price) => {
  ...
  try {
    ...
    alert("_shareRoom error : " + error.message)
  }
}
...
const _getMyRents = async () => {
  ...
  try {
    ...
    let rents = await contract.methods.getMyRents().call({
      from: user,
    })
    ...
    for (let i = 0; i < rents.length; i++) {
      myRents.push(rents[i])
    }

  return myRents
  } catch(error) {
    alert("_getMyRents : " + error.message)
  }
  return myRents;
}
...
const _getAllRooms = async () => {
  ...
  const roomNum = await contract.methods.roomId().call()
  for (let i = 0; i < parseInt(roomNum); i++) {
...
const rentRoom = async () => {
  ...
  // alert(checkInDate + " " + checkOutDate) // 1/4 -> 1/7 => 3, 6
...
const _rentRoom = async (roomId, currentYear, checkInDate, checkOutDate, price) => {
  ...
  try {
    let contract = await getRoomShareContract()
    rented = await contract.methods.checkRented(roomId, checkInDate, checkOutDate).call()
    room = await contract.methods.roomId2room(roomId).call()

    if (rented == 1) {
      throw 'rented'
    }

    if (price != (checkOutDate - checkInDate) * room.price) {
      throw 'price'
    }
    ...
    await contract.methods.rentRoom(roomId, currentYear, checkInDate, checkOutDate).send({from : user, gas: 3000000, value: price * 10 ** 15})
    alert("success")
  } catch(error) {
    // let data = error.data;
    // let txHash = Object.keys(data)[0]
    // let reason = data[txHash].reason

    if (error == 'price') {
      alert('incorret price')
    } else if (error == 'rented') {
      _recommendDate(roomId, checkInDate, checkOutDate)
    } else if (error == 'inactive') {
      alert('inactive')
    } else {
      alert("_rentRoom error : " + error.message)
    }
...
const _recommendDate = async (roomId, checkInDate, checkOutDate) => {
  ...
  try {
    let contract = await getRoomShareContract()
    let [alreadyIn, alreadyOut] = await contract.methods.recommendDate(roomId, checkInDate, checkOutDate).call()

    let checkIn = dateFromDay(currentYear, alreadyIn)
    let checkOut = dateFromDay(currentYear, alreadyOut)
    ...
    let alertMsg = 'already rented (' + checkInYear + '/' + checkInMonth + '/' + checkInDay + ' ~ ' + checkOutYear + '/' + checkOutMonth + '/' + checkOutDay+')'
    alert(alertMsg)
...
const getRoomRentHistory = async () => {
  ...
  let retOptJson = returnOptionsJSON()
  let id = retOptJson.id
  let history = []
  let contract = await getRoomShareContract()

  let rentHistory = await contract.methods.getRoomRentHistory(id).call()

  for (let i = 0; i < rentHistory.length; i++) {
    let checkInDate = dateFromDay(currentYear, rentHistory[i].checkInDate)
    let checkOutDate = dateFromDay(currentYear, rentHistory[i].checkOutDate)
    
    let checkInYear = checkInDate.getFullYear()
    let checkInMonth = checkInDate.getMonth()+1
    let checkInDay = checkInDate.getDate()

    let checkOutYear = checkOutDate.getFullYear()
    let checkOutMonth = checkOutDate.getMonth()+1
    let checkOutDay = checkOutDate.getDate()
    
    history.push({id: rentHistory[i].id, 
                checkInDate: checkInYear + "-"+checkInMonth + "-" + checkInDay,
                checkOutDate: checkOutYear + "-"+checkOutMonth + "-" + checkOutDay,
                renter: rentHistory[i].renter})
...
const markRoomAsInactive = async (_roomId) => {
  ...
  try {
    await contract.methods.changeRoomState(_roomId, false).send({from: user, gas: 3000000 });
    alert("success");
  } catch(error) {
    alert("markRoomAsInactive: "  + error.message);
  }
...
const intializeRoomShare = async (_roomId) => {
  ...
  try {
    await contract.methods.initializeRoom(_roomId).send({from: user, gas: 3000000});
    _updateRents();
    alert("success")
  } catch(error) {
    alert("intializeRoomShare: " + error.message)
  }
